...
- `POST /api/jobs` (multipart `file` + form options), `GET /api/queue`, `GET /api/jobs/{id}`, `DELETE /api/jobs/{id}` (running job → 409), `POST /api/jobs/{id}/pause`, `POST /api/jobs/{id}/resume`, `POST /api/jobs/{id}/cancel`, `GET /api/jobs/{id}/download/{kind}`, `GET /api/config` (translation providers).
