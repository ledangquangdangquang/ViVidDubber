...
- `POST /api/jobs` (multipart `file` + form options), `GET /api/queue`, `GET /api/jobs/{id}`, `DELETE /api/jobs/{id}` (running job → 409), `GET /api/jobs/{id}/download/{kind}`, `GET /api/config` (translation providers).
